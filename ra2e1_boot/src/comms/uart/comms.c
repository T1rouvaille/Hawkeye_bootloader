#include "../comms.h"
//#include "uart_stdout.h"    //WangJin 2021.04.06
#include "hal_data.h"       //WangJin 2021.04.06
#include "common_data.h"    //WangJin 2021.04.06
...
void comms_send(uint8_t * p_src, uint32_t len)
{
    ...
    if (FSP_SUCCESS == err)
    {
        while((UART_EVENT_TX_COMPLETE != g_uart_evt))
        {
            ;
        }
    }
...
fsp_err_t comms_read(uint8_t * p_dest, uint32_t *len, uint32_t timeout_milliseconds)
{
    ...
    if (FSP_SUCCESS == err)
    {
        while((UART_EVENT_RX_COMPLETE != g_uart_evt) && (true != g_timer0_event))
        {
            ;
        }
    }
